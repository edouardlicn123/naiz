...
> **当前版本**: `0.2.086`（`projects/demo-a2/config.toml`）
>
> **Bug 修复状态**: 已完成 7 轮穷举静态分析 + 针对性修复（R1–R7），另完成整合修复（Bug-1/Bug-2/i18n 管线/工具链去重）与**查 bug 系统升级**（Tier1 + Tier2，R8）与**拆分整合重构**（R9）与**日志/审计范围增强**（R10）与**cg 双形态语法**（R11）与**对象/参数括号规范化**（R12）与**全项目查 bug（R13）**与**再次全项目查 bug（R14）**与**针对漏网 bug 的 Tier3 规则扩充（R15）**与**拆分/封装机会落库（R16）**。
> - R16（拆分/封装机会落库，0.2.086）: 依 `symbol_audit` 日志 A/B 节（空）与 E 节簇分析甄别——拆分侧三处候选（nb.c 状态访问器 / layer_bg.c capture 三件套 / mouse.c 查询块）经逐文件核查均为既有拆分产物或共享私有 statics 的内聚单元，**保持不拆**（强行拆会令 NbState/三个 snapshot 静态全局化、破坏封装）。落地 4 项跨文件重复**封装**：①`hal_logf(fmt,...)` 变参格式化日志（`hal_pc98.c`，替代 save_io/save/nb_saveload/nb_commands/nb_scene/nb/layer_debug 共 28 处手写 snprintf+hal_log 两段式）②`palette_apply_dirty_rgb()` / `palette_apply_dirty_pal()`（`palette.c`，收口 nb_anim.c 像素轨/调色板轨三处脏 diff 调色板循环，prot 保护色可 NULL）③`mag_crop_rows()` / `mag_fill_palette()`（`mag.c` 文件内静态，堆/池双轨共享去 padding 裁剪与 256 色填充，与 C26/C29 语义吻合）④`layer_snapshot_alloc_dialog()`（`layer_internal.h` static inline，layer_bg×3 + layer_dialog 的对话框快照分配/OOM 日志收口）。新增公共符号：`hal_logf`、`palette_apply_dirty_rgb/pal`（+layer_internal 内联），均入 B90 索引；make 0 err/0 warn、pytest 全绿、fullaudit 全绿、symbol_audit A 节仍空
