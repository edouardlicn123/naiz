...
> **当前版本**: `0.2.090`（`projects/demo-a2/config.toml`）
>
> **Bug 修复状态**: 已完成 7 轮穷举静态分析 + 针对性修复（R1–R7），另完成整合修复（Bug-1/Bug-2/i18n 管线/工具链去重）与**查 bug 系统升级**（Tier1 + Tier2，R8）与**拆分整合重构**（R9）与**日志/审计范围增强**（R10）与**cg 双形态语法**（R11）与**对象/参数括号规范化**（R12）与**全项目查 bug（R13）**与**再次全项目查 bug（R14）**与**针对漏网 bug 的 Tier3 规则扩充（R15）**与**拆分/封装机会落库（R16）**与**背景CG换图自动复位对话框（R17）**与**运行目检修复：伪透明恢复+cg 换图自动关框（R20）**。
