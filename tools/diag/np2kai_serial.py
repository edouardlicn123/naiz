...
from tools.naiz_lib.np2kai_capture import EMULATOR, kill_stale_emulators
...
def run_emulator(hdi_path, serial_out, timeout_sec):
    ...
    stale = kill_stale_emulators()
    if stale:
        print(f"[serial] Killed {stale} stale emulator instance(s)")
...
def main():
    ...
    if "Hello from DOS!" in output:
        ...
        print("  Note: leftover emulator instances and a missing "
              "'--start' silently stall the serial chain; "
              "both are auto-handled on this run")
