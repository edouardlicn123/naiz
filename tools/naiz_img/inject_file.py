...
def _read_root(fs):
    return bytearray(fs._read_bytes(fs.root_offset, fs.root_sectors * fs.bytes_per_sector))
...
def inject(base_path, game_name, game_dir, output_path):
    print(f"Opening base HDI: {base_path}")
    img = HDIImage(base_path)
    fs = NAIZFatFS(img)

    print(f"  {fs.total_sectors} sectors, {fs.bytes_per_sector} B/sector, "
          f"{fs.cluster_size} B/cluster, FAT{fs.fat_type}")
    print(f"  Data offset: {fs.data_offset}, cluster_size: {fs.cluster_size}")
    print(f"  Root entries: {fs.root_entries}")
    print(f"  Max cluster: {fs._max_cluster}")

    # Read existing FAT into mutable list
    ...
    autoexec_new = (
        b'@ECHO OFF\r\n'
        b'PATH C:\\DOS;C:\\' + game_name.upper().encode() + b'\r\n'
        b'SET TEMP=C:\\DOS\r\n'
        b'SET DOSDIR=C:\\DOS\r\n'
        b'ECHO BOOT_OK > C:\\BOOTMARK.TXT\r\n'
        b'CD C:\\' + game_name.upper().encode() + b'\r\n'
        b'ENGINE.EXE\r\n'
    )
    ...
    if ae_off is None:
        print("  WARNING: AUTOEXEC.BAT not found")
    else:
        ae_cluster = struct.unpack_from('<H', root_data, ae_off + 26)[0]
        ae_size = struct.unpack_from('<I', root_data, ae_off + 28)[0]
        ae_chain = fs._get_cluster_chain(ae_cluster)
        ae_capacity = len(ae_chain) * fs.cluster_size
        print(f"  Found at root offset 0x{ae_off:x}, cluster={ae_cluster}, "
              f"size={ae_size}, chain_len={len(ae_chain)}, capacity={ae_capacity}")
        ...
        if len(autoexec_new) <= ae_capacity:
            ...
            print(f"  Overwrote {len(autoexec_new)} bytes in existing clusters")
        else:
            ...
            struct.pack_into('<I', root_data, ae_off + 28, len(autoexec_new))
            print(f"  Reallocated to {num_cl} new clusters, chain starts at {new_chain[0]}")

    _write_root(fs, root_data)

    # --- Step 2: Replace CONFIG.SYS ---
    config_new = b'FILES=30\r\nSHELL=\\COMMAND.COM /P\r\n'
    ...
    if cfg_off is not None:
        ...
        cfg_size = struct.unpack_from('<I', root_data, cfg_off + 28)[0]
        cfg_chain = fs._get_cluster_chain(cfg_cluster)
        cfg_capacity = len(cfg_chain) * fs.cluster_size
        print(f"  Found at cluster={cfg_cluster}, size={cfg_size}, capacity={cfg_capacity}")
        ...
        if len(config_new) <= cfg_capacity:
            ...
            print(f"  Overwrote {len(config_new)} bytes in existing clusters")
        else:
    ...
    # --- Step 3: Create or update DEMO-A1 directory ---
    ...
    # Build current directory content
    dir_data = bytearray()
    for c in dir_chain:
    ...
    for f in game_files:
        ...
        if fkey in existing_entries:
            ...
            esize = struct.unpack_from('<I', eraw, 28)[0]
            echain = fs._get_cluster_chain(ecluster)
            ecapacity = len(echain) * fs.cluster_size
            print(f"  {f}: exists at cluster={ecluster}, size={esize}, capacity={ecapacity}")
            ...
            if file_size <= ecapacity:
                ...
                print(f"    Overwrote {file_size} bytes in-place")
            else:
    ...
    written = len(game_files) - len([f for f in game_files if f.upper() in existing_entries])
    updated = len(game_files) - written
    total_free = fat_list.count(0)
    print(f"Done: {written} new, {updated} updated, {len(game_files)} total game files, "
...
def main():
    ...
    inject(args.base, args.game, game_dir, args.output)
