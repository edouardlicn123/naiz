...
from .inject_common import inject_into_hdi, DEFAULT_BASE
from naiz_lib import PROJECT_ROOT, GAMES_DIR, DISKS_DIR
...
def main():
    ...
    output = args.output
    if output is None:
        game_name = args.game.replace('/', '_').replace('\\', '_')
    ...
    if args.extract:
        if not os.path.isfile(output):
            print(f"Error: HDI not found: {output}. Run 'makegame.sh make <game>' first.")
            sys.exit(1)
        img = HDIImage(output)
        fs = NAIZFatFS(img)
        ...
        src = args.extract.replace('\\', '/').split('/')[-1]
        outname = f"{src}.{ts}"
