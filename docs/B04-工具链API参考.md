...
**模块**: `tools/naiz_lib/palette_utils.py`

调色板管线的核心工具函数，被 `pack_images.py` 和 `build_game.py` 共同使用。

### 3.1 常量

| 常量 | 默认值 | 说明 |
|------|--------|------|
| `MAGENTA_KEY` | `(255, 0, 255)` | 品红键色（透明背景） |
| `KEY_DIST_SQ` | `22500` | 品红阈值平方距离（150²） |
| `SKIN_WARM_R_OFFSET` | `4` | 肤色预热：R 增量 |
| `SKIN_WARM_G_DELTA` | `-2` | 肤色预热：G 减量 |
| `SKIN_R_MIN / G_LO / G_HI / B_LO / B_HI` | 200/160/245/120/220 | 肤色启发式范围 |
| `VALIDATE_GR_WARN` | `0.03` | G/R 比升高警告阈值 |
| `VALIDATE_DE_MAX` | `3.0` | ΔE 警告阈值 |

### 3.2 is_near_magenta

```python
def is_near_magenta(r, g, b, key=None, threshold_sq=None) -> bool
```

检查 (r,g,b) 是否接近键色。用于跳过半透明边缘像素（品红背景合成伪影）。

- `key`: 键色，默认 `MAGENTA_KEY`
- `threshold_sq`: 欧氏距离平方阈值，默认 `KEY_DIST_SQ`

### 3.3 find_skin_entries

```python
def find_skin_entries(palette) -> list[int]
```

返回调色板中符合肤色启发式范围的所有索引。

### 3.4 warm_skin_tone

```python
def warm_skin_tone(r, g, b, r_offset=None, g_delta=None) -> tuple
```

对肤色范围颜色做预热（R+4, G-2）以补偿 NP2kai RGB565 显示损耗。非肤色颜色原样返回。

- `r_offset`: R 增量，默认 `SKIN_WARM_R_OFFSET`
- `g_delta`: G 减量，默认 `SKIN_WARM_G_DELTA`

### 3.5 validate_skin_palette

```python
def validate_skin_palette(shared_palette, source_palettes,
                          gr_warn=None, de_max=None) -> list[str]
```

对比共享调色板与源 MAG 调色板的肤色条目，返回超过阈值的警告列表。

- `shared_palette`: `build_shared_palette()` 输出的 256 色调色板
- `source_palettes`: 源 MAG 调色板的可迭代对象
- `gr_warn`: G/R 比升高阈值，默认 `VALIDATE_GR_WARN`
- `de_max`: 颜色距离阈值，默认 `VALIDATE_DE_MAX`

每个警告包含：
- `ΔE > de_max`: 肤色条目映射后颜色偏差过大
- `ΔG/R > gr_warn`: 肤色条目 G/R 比异常升高（预示偏青）
...
LBA 计算公式：`cyl × heads × spt + head × spt + (sector - 1)`

**修复记录**（2026-06-19）：PC-98 IPL 分区表中 `entry[5]`（sector 字段）通常存储 0 表示 sector 1。
`detect_pc98()` 中 LBA 计算使用了 `sector` 而非 `(sector - 1)`，导致分区起始偏移少 1 个扇区。
修复：在 CHS→LBA 转换中对 sector 做 `max(sector, 1) - 1` 钳位。

base_msdos5_scsi_48m_clean.hdi 典型值：
- 分区起始：LBA 136 (cyl=1, head=0, sector=1)
- 类型 ID：`0x91`
- 大小：47520 扇区
...
```python
from naiz_img.fat import to_dos_name, make_fat_entry, build_fat_bytes

to_dos_name("AUTOEXEC.BAT")  # → (b'AUTOEXEC', b'BAT ')
make_fat_entry(name8, ext3, attr, cluster, size)  # → bytes(32)
build_fat_bytes(fat_list, fat_type, total_len)     # → bytes
```
...
内部辅助函数 `_inject_file_content()` 统一处理"分配-or-覆盖"逻辑。

```python
def _inject_file_content(fs, root_data, entry_offset, new_data, fat_list, alloc_fn, fat_eoc):
    """注入文件内容，参数 fat_eoc 指定 FAT12_EOC 或 FAT16_EOC 用于簇链终止。
    修复记录：原硬编码 FAT16_EOC，在 FAT12 镜像上导致链地址错乱。"""
```
...
```bash
python -m tools.naiz_img.inject -g demo-a2
python -m tools.naiz_img.inject -g mygame -o disks/mygame.hdi
python -m tools.naiz_img.inject -g demo-a2 --preview
python -m tools.naiz_img.inject -g demo-a2 --list-files
python -m tools.naiz_img.inject -g demo-a2 --extract DEMO-A1/ENGINE.LOG
```
...
| `-b, --base` | `tools_commercial/base_msdos5_scsi_48m_clean.hdi` | 基座 HDI |
...
**模块**: `tools/naiz_lib/mag_codec.py`

MAG (MAKI02) 图像格式的编解码单一来源，被 `mag_convert.py`、`pack_images.py` 共同使用。

### API

```python
from naiz_lib.mag_codec import (
    MAG_SIGNATURE,          # b"MAKI02  "
    expand_comp,            # 颜色分量扩展 (4/5/8-bit → 8-bit)
    build_ref_table,        # MAG 压缩参考表
    pixels_to_padded,       # 像素 → 填充字节
    encode_mag_stream,      # MAG 压缩流 (flag-A + flag-B + color)
    encode_mag,             # 完整 MAG 文件编码
    decode_mag_full,        # 完整 MAG 解码 → (pixels, w, h, palette, bpp, is_sprite)
    decode_mag_palette,     # 仅提取调色板
    to_dos_name,            # DOS 8.3 文件名转换
)
```
...
```python
encode_mag(pixels, width, height, palette, user_string=b"naiz\x1a", bpp=4) -> bytes
```

- `pixels`: 调色板索引列表，长度 = w*h
- `palette`: (r,g,b) 元组列表
- `bpp`: 4 或 8
- 返回完整 MAG 文件字节
...
```python
decode_mag_full(data) -> (pixels, width, height, palette, bpp, is_sprite) | None
```

- `data`: MAG 文件字节
- 返回 6 元组，或解码失败返回 None
