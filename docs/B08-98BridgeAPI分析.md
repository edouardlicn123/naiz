...
**HDI 头部格式验证**（当前 base_msdos5_scsi_48m_clean.hdi 的 HDI header）：

| 偏移 | 字段 | 值（base_msdos5_scsi_48m_clean.hdi） | 说明 |
|------|------|-------------------|------|
| 0x00 | reserved | 0 | |
...
**在 base_msdos5_scsi_48m_clean.hdi 上的行为**：
...
disk = open_image("base_msdos5_scsi_48m_clean.hdi")       # → HDIImage 实例
...
# 1. 打开基座 HDI
disk = open_image("~/msdos5/base_msdos5_scsi_48m_clean.hdi")
...
# 3. 覆写游戏文件
shutil.copy2("games/demo-a2/ENGINE.EXE",
             os.path.join(tmpdir, "demo-a2", "ENGINE.EXE"))

# 4. 打开基座 + write_back
disk = open_image("~/msdos5/base_msdos5_scsi_48m_clean.hdi")
fs = FATFilesystem(disk)
fs.write_back_from_directory(tmpdir, save_path="disks/demo-a2.hdi")
```
...
disk = HDIImage("base_msdos5_scsi_48m_clean.hdi")     # 直接调用，无需 registry
```
...
disk = open_image("base_msdos5_scsi_48m_clean.hdi")
fs = FATFilesystem(disk)
```
...
disk = HDIImage("base_msdos5_scsi_48m_clean.hdi")        # 直接构造
