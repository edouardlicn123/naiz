...
### 7.4 NP2kai 下 delay / 选项等待时间被拉长 3 倍

**现象**：NP2kai（模拟器）里 `delay 1.0` 实际等了约 3 秒，选项超时（600 帧）实际约 30 秒；真机上为名义值（60Hz）。

**根因（已接受语义，非 bug）**：`delay` 与选项超时按**主循环 pass 计数**（`sec×60` 帧逐 pass 递减），而 NP2kai 真实 pass 率仅 ~20-28Hz（≈ 1/3 于名义 60Hz）。这是引擎按帧推进的原生语义，脚本在真机与模拟器上的"节奏"本就不同。

**解决方案**：不改代码（H3，devdoc 107 §2.3）；选择等待的玩家策略是用选项正常点击，不依赖超时。凡需要机器无关真实时间的功能（打字机、动画帧、MIDI 时间轴），一律走 `hal_wallclock_smooth_ms()`（见 B19 §3.1），不得再按 pass 折算。
...
- P3 补丁未应用导致调色板自检失败
- **残留 NP2kai 实例占着 COM1**：旧 `wxnp21kai` 未退出（0.2.132 起工具链自动 `kill_stale_emulators`，`pkill -9 -x wxnp21kai` 精确匹配放行）
- **PTY slave 提前关闭**：`com1_m_o/com1_m_i` 指向的 PTY slave 端点若在模拟器 fork 前关闭，NP2kai 将打不开该端点 → 串口静默；工具链保持 slave_fd 全程开启
- **缺 `--start`**：无该参数时模拟器停在配置界面等手动点击，DOS 不启动 → 串口 0 字节（0.2.132 起 `--serial` 自动追加）

### 9.1b SERTEST.COM（`--porttest`）无输出

**现象**：`makegame.sh test demo-a2 --porttest` 打印 `No serial output received`。

**根因**：旧 `gen_com.gen_serialwrite` 用 **INT 14h**（BIOS 串口中断）——PC-9821 BIOS ROM **不含 INT 14h 处理函数**，字节永远发不出（0.2.129 曾误归因会话环境）。0.2.132 起改直接 uPD8251 端口 I/O（`0x30` 数据/`0x32` 命令状态），复位序列对齐 `serial.c`（0x40 复位 → 0x4E 9600 8N1 → 0x27 使能），逐字节 TXRDY 位等待后再发，输出 `Hello from DOS!\r\n` 全 17 字节到位。
