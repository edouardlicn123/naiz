...
> **状态**：**已归档** — 本文描述的是早期基于 INT 21h 文件写入的日志机制。当前引擎使用 `hal_log()` 通过串口（COM1, 9600 8N1）实时输出调试信息，不再创建 ENGINE.LOG 或使用 DOS 文件 I/O。保留仅供参考。
...
```bash
python3 -m tools.naiz_img.inject --game demo-a2 --extract ENGINE.LOG
```

执行流程：
1. 打开 `disks/demo-a2.hdi`
...
cp core/engine.exe games/demo-a2/
source venv/bin/activate
python3 -m tools.naiz_img.inject --game demo-a2 --yes   # 注入 HDI
./makegame.sh test demo-a2                  # 运行 NP2kai（engine 生成 ENGINE.LOG）
python3 -m tools.naiz_img.inject --game demo-a2 --extract ENGINE.LOG  # 提取
