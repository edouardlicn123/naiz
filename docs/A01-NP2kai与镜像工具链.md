...
游戏项目 (games/<name>/) + 配置 (tools/ref_config/)
    │
    ▼
tools/naiz_img/inject.py  ── 基于 base_msdos5_scsi_48m_clean.hdi 注入 ──▶  disks/<name>.hdi
    │
    ▼
...
| 基座镜像 | `tools_commercial/base_msdos5_scsi_48m_clean.hdi` |
| 输出镜像 | `disks/<game>.hdi` |
...
> **修复记录**（2026-06-19）：PC-98 IPL 分区表中 `entry[5]`（sector 字段）存储 0 表示 sector 1。
> `detect_pc98()` 此前使用 `sector` 而非 `(sector - 1)`，导致分区起始偏移少 1 个扇区。
> 修复：在 CHS→LBA 转换中对 sector 做 `max(sector, 1) - 1` 钳位。

**典型值**（base_msdos5_scsi_48m_clean.hdi）：
- 分区起始：cyl=1, head=0, sector=1 → LBA = 1×8×17 + 0×17 + 0 = 136
- 类型 ID：`0x91`
...
**base_msdos5_scsi_48m_clean.hdi 的 BPB**：
...
`tools/naiz_img/` 下 FAT 实现拆分为四文件：`fat_entries.py`（`FileEntry` 目录项 + `make_fat_entry`）、`fat_table.py`（FAT 表构建 + 簇链分配）、`fat_fs.py`（`NAIZFatFS` 类）、`fat.py`（re-export 兼容层，`NAIZFatFS` 等符号原样转发）。参考 98Bridge `fat_fs.py` 的设计思路独立实现。
...
**`_inject_file_content()` 更新**（2026-06-19）：新增 `fat_eoc` 参数指定 FAT12_EOC（`0xFF8`）或 FAT16_EOC（`0xFFF8`），兼容 FAT12/FAT16 双格式。修复前硬编码 FAT16_EOC，在 FAT12 基座镜像上导致簇链尾标记错误。
...
### base_msdos5_scsi_48m_clean.hdi 实际检测值
...
```bash
python -m tools.naiz_img.inject --game demo-a2
python -m tools.naiz_img.inject --game mygame -o disks/mygame.hdi
python -m tools.naiz_img.inject --game demo-a2 --preview   # 预览
python -m tools.naiz_img.inject --game demo-a2 --list-files # 列出基座镜像文件
```
...
```bash
./makegame.sh make demo-a2     # 制作 HDI
./makegame.sh test demo-a2     # 模拟器启动测试
...
**解决**：分配目录簇后立即写入 `b'\x00' * fs.cluster_size` 清零。已在 `inject_common.py` 中实现：`_zero_cluster(fs, cluster)` 在每次新链分配后自动清零所有簇数据。
