 ...
 *
 * This buffer is the DURABLE composite: it is not repainted on every pass,
 * so the transient software-cursor arrow must NEVER be written into it (a
 * baked arrow survives dither holes and later re-blits = move residue,
 * devdoc 111).  dialog_layer_blit blends the arrow into a throwaway
 * write-only copy (dialog_blit_tmp) instead. */
static unsigned char *dialog_layer = NULL;
/* Throwaway blit copy (480 x LAYER_DIALOG_H): dialog_layer + the spliced
 * arrow, written to VRAM and discarded — keeps the durable composite pure.
 * Lazily allocated with the other dialog buffers; NULL degrades the blit to
 * the pass-end cursor redraw (no splice). */
static unsigned char *dialog_blit_tmp = NULL;
/* Non-zero when dialog area is currently drawn on screen. */
static unsigned char dialog_drawn = 0;

#ifdef NAIZ_TRACE_CURSOR
static unsigned long nz_scale_seq;
#endif
...
/* Fill rectangle with dialog background style (solid or dither).  Writes
 * straight to VRAM — report the rect to the software cursor layer first
 * (devdoc 108 layer contract; e.g. the save/load slot grid). */
void fill_dialog_bg(int x, int y, int w, int h)
{
    hal_mouse_touch_cursor(x, y, w, h);
    if (g_dialog_style & 1)
...
/* Blit the dialog composite to VRAM.
 * The composite vram_write covers the whole dialog rect pixel-for-pixel, so
 * report it via the OPAQUE touch (devdoc 109): a cursor fully inside the box
 * is left for the write to clear and then redrawn immediately without a
 * VBLANK wait — killing the per-pass full-frame flicker during typewriter
 * reveal.  The backdrop box is still erased first only when the cursor box
 * straddles the rect border.  Transparent/partial writers must keep touch.
 * The whole write is slow through the bank-switched DOS/4GW VRAM window, so
 * when the cursor box sits fully inside the rect the arrow is additionally
 * spliced into a THROWAWAY copy of the composite FIRST
 * (cursor_composite_splice onto dialog_blit_tmp, devdoc 110 + 111): the
 * single vram_write then lands with the cursor already on screen — zero
 * absence window, no extra VRAM traffic, no ghost (background = the box
 * pixels from the durable composite).  The arrow is NEVER written into the
 * durable dialog_layer: it is not repainted every pass, so a baked arrow
 * would survive dither holes and later re-blits as move residue (devdoc
 * 111).  When the throwaway copy is unavailable (OOM) the splice is skipped
 * and the pass-end redraw covers the cursor as before 0.2.137. */
void dialog_layer_blit(void)
{
    if (dialog_layer) {
        hal_mouse_touch_cursor_opaque(LAYER_DIALOG_X, LAYER_DIALOG_Y,
                                      LAYER_DIALOG_W, LAYER_DIALOG_H);
        if (dialog_blit_tmp) {
            memcpy(dialog_blit_tmp, dialog_layer, LAYER_DIALOG_W * LAYER_DIALOG_H);
            if (!cursor_composite_splice(dialog_blit_tmp, LAYER_DIALOG_W,
...
void layer_dialog_show(void)
{
    if (!dialog_layer) {
        ...
        dialog_blit_tmp = layer_snapshot_alloc_dialog("dialog_blit_tmp");
    }
    if (!dialog_layer) {
        /* OOM degraded path: paint the box straight to VRAM (legacy). */
        hal_mouse_touch_cursor(LAYER_DIALOG_X, LAYER_DIALOG_Y,
                               LAYER_DIALOG_W, LAYER_DIALOG_H);
...
void layer_dialog_reset(void)
{
    ...
    if (dialog_blit_tmp) { free(dialog_blit_tmp); dialog_blit_tmp = NULL; }
    dialog_drawn = 0;
