 ...
 * from_mainmenu=1: Back/ESC exits via scene_switch() (return to caller scene);
...
static int save_load_confirm_action(int slot)
{
    if (g_confirm_is_load) {
        if (save_request(SAVE_OP_SLOT_LOAD, slot) != 0) {
        ...
        if (confirm) {
            ...
            if (focus_on_back) {
                if (hal_kbd_is_down(KC_UP)) {
                    ...
                    if (from_mainmenu) {
                        scene_switch(orig_nb[0] ? orig_nb : "mainmenu.nb", SCENE_SWITCH_MENU);
                    } else {
                        if (save_request(SAVE_OP_RESTORE_TEMP, -1) != 0)
...
/*=== Save/load request funnel =============================================*/

/* Single access point for every save/load request (scene-switch scene loads
 * that are not save/load belong to scene switching, not this funnel). */
int save_request(SlotOp op, int slot)
{
    switch (op) {
    case SAVE_OP_CAPTURE_TEMP:
        return save_game_temp();
    case SAVE_OP_RESTORE_TEMP:
        return load_game_temp();
    case SAVE_OP_SLOT_SAVE:
        save_game_slot(slot);
        return 0;
    case SAVE_OP_SLOT_LOAD:
        return load_game_slot(slot);
    case SAVE_OP_OPEN_SAVE:
        save_dialog_menu();
        return 0;
    case SAVE_OP_OPEN_LOAD:
        scene_switch("loadscen.nb", SCENE_SWITCH_HOTKEY);
        return 0;
    case SAVE_OP_OPEN_MAINMENU:
        scene_switch("mainmenu.nb", SCENE_SWITCH_HOTKEY);
        return 0;
    }
    hal_logf("[SAVELOAD] save_request: unknown op %d\r\n", (int)op);
    return -1;
}
