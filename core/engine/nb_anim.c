...
int anim_tick(void)
{
     ...
     *
     * Clock source (devdoc 107 / 0.2.134): the HAL smooth virtual clock
     * hal_wallclock_smooth_ms().  Raw hal_wallclock_ms() per-pass deltas are
     * chunked under NP2kai (frozen 3-6s, then a single +1000..+6000ms jump) —
     * that froze the animation during the holes and burst the whole sequence
     * through the steps>180 clamp on catch-up.  The smooth clock advances at
     * pass cadence through the holes with deltas bounded per pass, so steps
     * never accumulates a stall surplus.  Calibration: the first call after
     * arm records the baseline timestamp via tick_armed (not last_ms==0,
     * which is ambiguous at clock reset); the smooth clock is monotonic, so
     * no wrap handling is needed.
     */
